...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        action="store",
        default=None,
        help="Specific model to test (if not provided, tests current deployed model)"
    )
...
def test_model(request):
    """Get specific model to test from CLI option."""
...
def api_key(config):
    ...
    return os.environ.get("OPENWEBUI_API_KEY") or config["api"]["key"]


@pytest.fixture(scope="session")
def api_base_path(config):
    """Get Open WebUI API base path."""
    return config["api"]["base_path"]
